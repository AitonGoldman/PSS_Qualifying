...
## Flask configuration
Please read the following parts of the flask QuickStart before continuing - they will explain Flask routes and Flask blueprints :
- The "Minimal application" section of the quickstart is an explanation of a minimal flask application
  - http://flask.pocoo.org/docs/1.0/quickstart/#a-minimal-application
- The "Routing" section of the quickstart is a brief explanation of how Flask routes work
  - http://flask.pocoo.org/docs/1.0/quickstart/#routing
- The "Variable Rules" section of the quickstart is a brief explanation of how variables work in routes
  - http://flask.pocoo.org/docs/1.0/quickstart/#variable-rules
- The "Create a blueprint" in the Blueprint and Views section is a brief explanation of Flask blueprints 
  - http://flask.pocoo.org/docs/1.0/tutorial/views/#create-a-blueprint
...
The skeleton uses the `config()` method from the `decouple` module to retrieve environment variables.  It is only used for getting environment variables in the skeleton, but it allows for much more advanced configuration management ( see here for docs - https://github.com/henriquebastos/python-decouple/).
...
The SQLAlchemy object comes from the Flask-sqlalchemy package ( docs here : http://flask-sqlalchemy.pocoo.org/2.3/ ).  The object does many things ( and will be discussed in more detail later ) but for now it represents a connection to a database. 
...
`get_event_route()` takes the results of `get_event()` (a dict representing the found SQLAlchemy Event object) and `jsonify`-ies it and returns the result.  `jsonify()` takes a dict, converts it to a json string, and then creates a `Flask Response` ( http://flask.pocoo.org/docs/1.0/api/#flask.Response ) with the json string as the body of the response.   All Flask routes must return a `Flask Response` or raise a http exception.
...
If no event is found, a `BadRequest` exception is raised.  In this case, the exception will rise up the stack until Flask converts it to a 400 http response with the body "Invalid event id".  All business logic code should follow this pattern of raising http exceptions when problems are encountered (see here for list of http exceptions - http://werkzeug.pocoo.org/docs/0.14/exceptions/) 

`get_event()` is a separate function from `get_event_route()` - it is separate because it allows us to easily unit test the business logic.  Because the `TableProxy` is an argument to `get_event()`, it is easy to mock it and thus test `get_event()`.  All business logic code should follow this pattern : seperate the business logic from the functions that are decorated as Flask routes, and pass any "global" variables into the functions that implements the business logic.
...
The `__init__()` function will call `generate_event_model()` which generates a SQLAlchemy model class.  A SQLAlchemy models represent a table in the database and is used to run queries against the database for that table.  For more information on how we use SQLAlchemy to query the database, see these docs for flask-sqlalchemy (a package that integrates Flask and SQLAlchemy): http://flask-sqlalchemy.pocoo.org/2.3/quickstart/ and http://flask-sqlalchemy.pocoo.org/2.3/queries/ 
...
Note that `to_dict()` is a method we added to the SQLAlchemy model class via a mixin - the details of the mixin will be discussed later.

Three patterns which all proxies should follow : 
...
- The `EventsProxy` methods do not commit changes to the database - this is because `get_event()` doesn't make any changes that need to be commited.  But if there were changes to commit, it would still not issue the commit command.  This is because there will be situtions where the business logic will need to call multiple Proxy functions, and we don't want a commit to happen half way through.  The `TableProxy` provides a method that issues the commit command, and it is expected that the business logic will call that method once it has completed all it's Proxy calls. 

Next let's look at the Event SQLAlchemy model...

## Event model
     
SQLalchemy models are classes that represent tables in a database and the relationships between those tables (i.e. one-to-one, one-to-many, many-to-many).  For more details on SQLAlchemy models, see the docs for flask-sqlalchemy (http://flask-sqlalchemy.pocoo.org/2.3/models/).  A single `Event` (i.e. papa 20) is represented by the `Events` model in `models/Events.py`
...
The `SerializerMixin` is a mixin that provides the `to_dict` method on the SQLAlchemy class, which converts query results into a dict.  `to_dict()` has a number of options that we will eventually use, but for now we are just using it without any arguments. 
...
Unit tests are located under `tests/unit` and integration tests are located under `tests/integration`.  The following command will run the unit tests : `python -m unittest discover -s tests/unit/`.  The following command will run the integration tests : `python -m unittest discover -s tests/integration/`.  Please review the unit tests and integration tests for the skeleton to get an understanding of what is expected from both types of tests.

## Next Steps

Congratulations!  You now have an understanding of the architecture of PSS_Qualifying.  There are several things to do next after checking out the master branch
