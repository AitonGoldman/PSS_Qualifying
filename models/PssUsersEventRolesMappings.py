...
def generate_pss_users_event_roles_mappings_model(db_handle):
    class PssUsersEventRolesMappings(db_handle.Model, SerializerMixin):
        ...
        event_roles = db_handle.relationship('EventRoles',lazy="joined",uselist=False)
        events = db_handle.relationship('Events',lazy="joined",uselist=False)
